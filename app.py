...
# Inyección de estilos CSS usando los colores oficiales de FedEx
st.markdown("""
    <style>
...
class VapaEngine:
    ...
    def process_file(file):
        ...
        try:
            ...
            # NUEVO: Agregar fecha de carga
            df['Fecha de Carga'] = datetime.now().strftime('%Y-%m-%d')
...
if st.session_state.history:
    ...
    with col5:
        st.markdown(f"<div class='metric-box' style='border-left-color: #4D148C;'><strong>En Bodega (Inventario)</strong><br><span style='font-size:26px;font-weight:bold;color:#FFFFFF;'>{sin_mov}</span></div>", unsafe_allow_html=True)
    ...
    chart_data = pd.DataFrame({
        ...
        "Color": ["#00B4D8", "#FFB703", "#E63946", "#06D6A0", "#4D148C"]
    })
    ...
    # NUEVO: Controles para búsqueda y filtro SIP alineados
    ...
    # Columnas esenciales (agregamos Shipper Name, status y Fecha)
    cols_to_check = ['Tracking Number', 'Shipper Name', 'status', 'Status', 'SIPS Date Time Loc Latest', 'STAT 50 Latest', 'STAT 53 All', 'DEX All', 'Fecha de Carga']
    cols_to_show = [c for c in cols_to_check if c in df_bodega.columns]
    
    display_df = df_bodega[cols_to_show].copy()
    
    # Aplicar el filtro de SIP si la columna existe en el documento
    if filtro_sip:
        if 'status' in display_df.columns:
            display_df = display_df[display_df['status'].astype(str).str.upper() == 'SIP']
        elif 'Status' in display_df.columns:
            display_df = display_df[display_df['Status'].astype(str).str.upper() == 'SIP']
    ...
    # NUEVO: Aplicar colores de FedEx por cliente
    def color_fedex_cliente(row):
        cliente = str(row.get('Shipper Name', ''))
        color = ''
        if 'Tricot' in cliente:
            color = 'background-color: #FF6600; color: white;' # Naranja FedEx
        elif any(c in cliente for c in ['Cruz Verde', 'Intercarry', 'Farmacias Ahumada']):
            color = 'background-color: #4D148C; color: white;' # Morado FedEx
